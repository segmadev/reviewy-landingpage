...
export default function ChooseTemplatePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Nav */}
      <nav className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between sticky top-0 z-10">
        <Link to="/">
          <img src="/asstes/onwhite-logo.png" alt="ReviewyMe" className="h-7 sm:h-8 w-auto" />
        </Link>
        <div className="flex items-center gap-2 sm:gap-3">
          <span className="text-xs sm:text-sm text-gray-500 hidden sm:block">
            {TEMPLATES.find(t => t.id === selected)?.name} selected
          </span>
          <Button size="sm" onClick={handleContinue} className="text-xs sm:text-sm px-3 sm:px-4">
            Use This →
          </Button>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-mint-50 border border-primary/20 text-primary text-xs font-semibold px-3 py-1 rounded-full mb-3 sm:mb-4">
            <Palette className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
            Choose Your Style
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Pick a Template</h1>
        ...
        {/* Template grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-5 mb-8 sm:mb-10">
          {TEMPLATES.map((tpl, i) => {
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-5">
            <div className="min-w-0">
              ...
              <p className="text-xs sm:text-sm text-gray-400">{TEMPLATES.find(t => t.id === selected)?.description}</p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={() => setShowModal(true)}
                title="Full preview"
                className="p-1.5 sm:p-2 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 hover:border-gray-300 transition-all"
              >
          ...
          <div className="flex gap-4 sm:gap-6 flex-col lg:flex-row">
            ...
            {/* Large preview */}
            <div className="flex-1 overflow-auto rounded-xl border border-gray-100 bg-gray-50 flex justify-center p-3 sm:p-4" style={{ maxHeight: '600px' }}>
              {(() => {
